...
OUTPUT_DIR    = Path("results/000792SZ_20260407")
...
def build_report(ticker, trade_date, snapshot, final_state, decision) -> str:
    ...
    return f"""# 000792.SZ 投资分析报告
## 青海盐湖工业 · 目标交易日：{trade_date}
...
---
*Generated by TradingAgents Framework · 000792.SZ {trade_date} Deep Analysis*
"""
...
def main():
    print("=" * 70)
    print(f"  000792.SZ (青海盐湖工业) 深度分析")
    print(f"  目标日期：{TRADE_DATE}  |  研究深度：Deep (5轮)")
    print(f"  运行时间：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 70)

    # Step 1: 数据快照
    print("\n[1/3] 拉取 yfinance 数据快照...")
    ...
    # Step 3: 保存报告
    report_path = OUTPUT_DIR / f"000792SZ_analysis_{TRADE_DATE}.md"
